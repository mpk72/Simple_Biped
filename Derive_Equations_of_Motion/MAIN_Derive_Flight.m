...
r1 = x*i + y*j; %Position of Foot One

r0 = r1-L1*a1;         % Position of the Hip (Absolute)
...
%r1 = x*i + y*j;  %Position of Foot One (Absolute)
dr1 = dx*i + dy*j;
ddr1 = ddx*i + ddy*j;

%r0 = r1-L1*a1;         % Position of the Hip (Absolute)
dr0 = dr1 - dL1*a1 - L1*da1;
ddr0 = simplify(ddr1 - ddL1*a1 - 2*dL1*da1 - L1*dda1);
...
sum_of_forces = simplify(N1*b1 - m*g*j + F1*a1);
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
sum_of_moments = simplify(-Thip*k - L1*N1*k);
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
disp(' -> Writing ''dynamics_flight.m''')
States = cell(12,2);
...
States(7,:) = {'dx','(m) Foot One Horizontal Velocity'};
States(8,:) = {'dy','(m) Foot One Vertical Velocity'};
...
Actuators(3,:) = {'Thip','(Nm) Hip torque applied to Leg Two from Leg One'};
...
Write_Dynamics_Flight(input);

%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
%                  Calculate other useful things                          %
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
Energy.Potential = simplify(m*g*dot(r2,j) + m*g*dot(r1,j) + M*g*dot(r0,j));
Energy.Kinetic = simplify(0.5*m*norm(dr2).^2 + 0.5*m*norm(dr1).^2 + 0.5*M*norm(dr0)^2);
