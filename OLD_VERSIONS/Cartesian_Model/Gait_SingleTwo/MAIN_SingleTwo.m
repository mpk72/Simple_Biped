...
%Constant states
auxdata.constant.x2 = 0;
auxdata.constant.y2 = 0;


auxdata.phase{1} = 'S2';  %Used for plotting and animation
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    P.Bnd.State.x1 = STEP_LENGTH(UPP)*[-1;1];
    P.Bnd.State.y1 = [0;0.5];
    P.Bnd.State.dx1 = 3*[-1;1];
    P.Bnd.State.dy1 = 1*[-1;1];
    
    config.initialstate.x1 = STEP_LENGTH(UPP)*[-0.9; -0.7];
    config.initialstate.y1 = [0;0];
    config.initialstate.dx1 = 0.7*[-1;1];
    config.initialstate.dy1 = 0.2*[-1;1];
    
    config.finalstate.x1 = STEP_LENGTH(UPP)*[0.7; 0.9];
    config.finalstate.y1 = [0;0];
    config.finalstate.dx1 = 0.7*[-1;1];
    config.finalstate.dy1 = 0.2*[-1;1];
...
P.Bnd.Actuator.T2 = AnkleTorque*[-1;1]; % (Nm) External torque applied to Leg One
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
iphase = 1;
stateBound = convertPartial(P.Bnd.State,'S2');
controlBound = convertPartial(P.Bnd.Actuator,'S2');
...
%-------------------------------------------------------------------------%
setup.name = 'Gait_SingleTwo';
setup.functions.continuous = @Continuous_SingleTwo;
setup.functions.endpoint = @Endpoint_SingleTwo;
